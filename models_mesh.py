...
class Mapping(layers.Layer):
    ...
    def __init__(self, n_mapping, w_dim=256, **kwargs):
        ...
        self.y_emb = LabelEmbedding(w_dim, name='y_emb')
...
def Generator(input_shape=[(256, ), (468, 3)], ngf=64, nc=3, im_size=256):
    nfc_multi = {
        4: 16,
        8: 8,
        16: 4,
    ...
    x = keras.Input(shape=input_shape[0])
    y = keras.Input(shape=input_shape[1])

    w = Mapping(8)(x, y)

    feat_4 = ConstLayer(n_const_fmap=256)(x)
    feat_8 = up_block_comp(feat_4, nfc[8], w)
    feat_16 = up_block(feat_8, nfc[16], w)
    feat_32 = up_block_comp(feat_16, nfc[32], w)

    feat_64 = se_block(feat_4, up_block(feat_32, nfc[64], w), nfc[64])
    feat_128 = se_block(feat_8, up_block_comp(feat_64, nfc[128], w), nfc[128])
    feat_256 = se_block(feat_16, up_block(feat_128, nfc[256], w), nfc[256])
    ...
    if im_size == 256:
        ...
        return Model(inputs=[x, y], outputs=img_256)

    feat_512 = se_block(feat_32, up_block_comp(feat_256, nfc[512], w),
                        nfc[512])
    if im_size == 512:
        img_512 = to_big(feat_512)
        img_512 = layers.Activation('tanh')(img_512)
        return Model(inputs=[x, y], outputs=img_512)

    feat_1024 = up_block(feat_512, nfc[1024], w)

    img_1024 = to_big(feat_1024)
    img_1024 = layers.Activation('tanh')(img_1024)

    model = Model(inputs=[x, y], outputs=img_1024)
    return model
